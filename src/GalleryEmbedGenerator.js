/**
 * Herramienta para generar códigos de embebido (para galerías
 * de imágenes) de forma interactiva al estilo YouTube
 *
 * @author Pablo Enrici
 */
...
(function(window, document, undefined){
  ...
  var url,
      ...
      requiredFields   = ['rec_id', 'baseurl', 'width', 'height', 'autoplay', 'time', 'skin'],

      // Opciones de configuración por defecto
      defaults = {
        formSelector  : "[data-galleryembedgenerator]",
        targetSelector: "[data-galleryembedgenerator-target]"
      };
  ...
  function init(options) {
    ...
    if (options !== null && typeof options === 'object') {
      ...
      console.warn("Parámetros para GalleryEmbedGenerator.init() inválidos.");
      return;
    }
  ...
  function hasAllRequiredFields(form) {
    ...
    if (missing.length !== 0) {
      console.warn("No se puede inicializar GalleryEmbedGenerator porque faltan estos campos en el formulario: " + missing.join(', '));
    }
  ...
  function updateForm(changedElement) {
    switch (changedElement.name) {
      ...
      case "autoplay":
        form.elements.time.readOnly = !changedElement.checked;
        form.elements.time.value    =  changedElement.checked ? 2000 : "";
        break;
  ...
  /**
   * Devuelve la URL del servicio de galerías de imágenes.
   *
   * @return {string}
   */
  function getUrl() {
    ...
    url +=  form.elements.time.value             ? "&autoplay=" + form.elements.time.value : "";
    url +=  form.elements.skin.value != "twelve" ? "&skin="     + form.elements.skin.value : "";
